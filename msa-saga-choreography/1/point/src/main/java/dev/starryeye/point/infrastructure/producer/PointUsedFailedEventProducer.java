...
public class PointUsedFailedEventProducer {

    private final KafkaTemplate<String, PointUsedFailedEvent> kafkaTemplate;

    public void send(PointUsedFailedEvent event) {

        /**
         * 키를 지정하면 항상 동일한 파티션으로 메시지가 발행되어 동일한 키를 가진 이벤트는 순서대로 처리됨을 보장 받을 수 있다.
         */
        kafkaTemplate.send(
                "point-used-failed", // topic name
